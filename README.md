...
3. Der Server stellt anschließend das Plugin-Paket über `plugin-download.php` bereit (z. B. für `wp plugin install <URL>`).
4. Stelle sicher, dass `server/api.php` und `server/plugin-download.php` über HTTPS erreichbar sind und per HTTP-Auth geschützt werden.

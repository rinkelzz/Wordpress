...
$serpConfigured = trim((string)($config['serp']['api_key'] ?? '')) !== '';
...
$sql = "
    SELECT i.id, i.site_name, i.site_url, i.site_group, i.site_token, i.last_seen,
           ...
           r.rank_keywords, r.rank_missing, r.rank_best, r.rank_updated
    ...
    LEFT JOIN (
        SELECT installation_id,
               COUNT(*) AS rank_keywords,
               SUM(CASE WHEN position IS NULL OR position = 0 THEN 1 ELSE 0 END) AS rank_missing,
               MIN(CASE WHEN position IS NULL OR position = 0 THEN 10000 ELSE position END) AS rank_best,
               MAX(retrieved_at) AS rank_updated
        FROM installation_rankings
        GROUP BY installation_id
...
$detailRankings    = [];
$detailRankingUpdated = null;
$rankingResultLimit = (int)($config['serp']['results'] ?? 10);
if ($rankingResultLimit < 1) {
    $rankingResultLimit = 10;
}
$detailRankingUpdatedDisplay = '';

if ($detailRow) {
    $rankingStmt = $pdo->prepare('SELECT keyword, position, title, link, retrieved_at FROM installation_rankings WHERE installation_id = :id ORDER BY keyword ASC');
    $rankingStmt->execute([':id' => (int) $detailRow['id']]);
    $detailRankings = $rankingStmt->fetchAll() ?: [];

    foreach ($detailRankings as $rankingRow) {
        $retrievedAt = $rankingRow['retrieved_at'] ?? '';
        if ($retrievedAt === '' || $retrievedAt === null) {
            continue;
        }

        if ($detailRankingUpdated === null || strcmp($retrievedAt, $detailRankingUpdated) > 0) {
            $detailRankingUpdated = $retrievedAt;
        }
    }

    if ($detailRankingUpdated !== null) {
        try {
            $detailRankingUpdatedDisplay = (new DateTimeImmutable($detailRankingUpdated, new DateTimeZone('UTC')))->format('d.m.Y H:i');
        } catch (Exception $exception) {
            $detailRankingUpdatedDisplay = $detailRankingUpdated;
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .detail-meta-secondary { font-size: 0.8rem; color: #6b7280; margin-top: 0.6rem; }
        .detail-empty { margin: 0; font-size: 0.9rem; color: #6b7280; }
        .table-subtext { font-size: 0.75rem; color: #6b7280; margin-top: 0.25rem; }
        .ranking-cell { min-width: 170px; }
    </style>
</head>
<body>
    ...
    <?php if (! $serpConfigured): ?>
        <p class="latest-version-hint">Google-Rankings werden erst ermittelt, wenn in der Server-Konfiguration ein SerpAPI-Schlüssel hinterlegt ist.</p>
    <?php endif; ?>
            ...
            <?php else: ?>
                ...
                <div class="detail-grid">
                    ...
                    <div class="detail-box">
                        ...
                        <?php if (! $serpConfigured): ?>
                            <p class="detail-empty">Nicht aktiviert.</p>
                        <?php elseif (empty($detailRankings)): ?>
                            <p class="detail-empty">Noch keine Ranking-Daten vorhanden.</p>
                        <?php else: ?>
                            <ul class="detail-list">
                                <?php foreach ($detailRankings as $rankingRow): ?>
                                    <?php
                                    $keyword  = (string)($rankingRow['keyword'] ?? '');
                                    $position = isset($rankingRow['position']) ? (int)$rankingRow['position'] : null;
                                    $link     = (string)($rankingRow['link'] ?? '');
                                    ?>
                                    <li>
                                        <span class="detail-highlight"><?php echo htmlspecialchars($keyword, ENT_QUOTES, 'UTF-8'); ?>:</span>
                                        <?php if ($position !== null && $position > 0): ?>
                                            #<?php echo $position; ?>
                                        <?php else: ?>
                                            Keine Treffer in den Top <?php echo $rankingResultLimit; ?>
                                        <?php endif; ?>
                                        <?php if ($link !== ''): ?>
                                            – <a href="<?php echo htmlspecialchars($link, ENT_QUOTES, 'UTF-8'); ?>" target="_blank" rel="noopener">Snippet</a>
                                        <?php endif; ?>
                                    </li>
                                <?php endforeach; ?>
                            </ul>
                            <?php if ($detailRankingUpdatedDisplay !== ''): ?>
                                <p class="detail-meta detail-meta-secondary">Letzte Aktualisierung: <?php echo htmlspecialchars($detailRankingUpdatedDisplay, ENT_QUOTES, 'UTF-8'); ?> UTC</p>
                            <?php endif; ?>
                        <?php endif; ?>
                    </div>
                    <div class="detail-box">
    ...
    <table>
        ...
        <tbody>
            <?php if (empty($rows)): ?>
                <tr>
                    <td colspan="13">Keine Installationen gefunden.</td>
                </tr>
            <?php else: ?>
                <?php foreach ($rows as $row): ?>
                    <?php
                    [$badges, $state] = wp_monitor_row_status($row);
                    $rankKeywords   = (int)($row['rank_keywords'] ?? 0);
                    $rankMissing    = (int)($row['rank_missing'] ?? 0);
                    $rankFound      = max(0, $rankKeywords - $rankMissing);
                    $rankBestRaw    = isset($row['rank_best']) ? (int)$row['rank_best'] : 10000;
                    $rankBest       = $rankBestRaw >= 10000 ? null : $rankBestRaw;
                    $rankUpdatedRaw = (string)($row['rank_updated'] ?? '');
                    $rankUpdatedFormatted = '';

                    if ($rankUpdatedRaw !== '') {
                        try {
                            $rankDate             = new DateTimeImmutable($rankUpdatedRaw, new DateTimeZone('UTC'));
                            $rankUpdatedFormatted = $rankDate->format('d.m.Y H:i');
                        } catch (Exception $exception) {
                            $rankUpdatedFormatted = $rankUpdatedRaw;
                        }
                    }
                    ?>
                    <tr class="state-<?php echo htmlspecialchars($state, ENT_QUOTES, 'UTF-8'); ?>">
                        <td>
                            <?php
                            $adminUrl = '';
                        ...
                        <td class="ranking-cell">
                            <?php if (! $serpConfigured): ?>
                                –
                                <div class="table-subtext">Nicht aktiviert</div>
                            <?php elseif ($rankKeywords === 0): ?>
                                –
                                <div class="table-subtext">Keine Daten</div>
                            <?php else: ?>
                                <?php if ($rankBest !== null): ?>
                                    #<?php echo $rankBest; ?>
                                <?php else: ?>
                                    Keine Treffer
                                <?php endif; ?>
                                <div class="table-subtext">
                                    <?php echo $rankFound; ?>/<?php echo $rankKeywords; ?> Keywords
                                    <?php if ($rankUpdatedFormatted !== ''): ?>
                                        – <?php echo htmlspecialchars($rankUpdatedFormatted, ENT_QUOTES, 'UTF-8'); ?> UTC
                                    <?php endif; ?>
                                </div>
                            <?php endif; ?>
                        </td>
